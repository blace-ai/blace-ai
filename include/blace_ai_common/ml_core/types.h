 ...
 */
enum DeviceEnum : int { CPU = 0, CUDA = 1, MPS = 2, DML = 3 };
 ...
 */
enum PADDING_MODE { REPLICATION = 0, REFLECTION, CONSTANT };

/**
 * Model offloading strategy.
 * NO_OFFLOADING: Models are kept on device.
 * TO_RAM: Unused models are moved to RAM.
 * FULL_UNLOAD: Unused models are completely unloaded.
 */
enum MODEL_OFFLOADING_STRATEGY { NO_OFFLOADING = 0, TO_RAM, FULL_UNLOAD };
...
/**
 * Backends for model inference. A backend is composed of a provider, a device
 * and a precision.
 */
enum Backend {
...
/**
 * Available Frameworks for model inference.
 */
enum Framework { TORCHSCRIPT, ONNXRUNTIME };
...
struct ModelInferenceArgs {
  /**
   * The backends to run the inference on. It will run the execution with the
   * first backend from the list supported by the model.
   */
  ...
  MODEL_OFFLOADING_STRATEGY model_offloading_strategy =
      MODEL_OFFLOADING_STRATEGY::FULL_UNLOAD;
  /**
...
struct MultisampleInferenceArgs {
  ...
  int samples = 1;
  /**
   * Allow mirroring of the input.
   */
  int allow_mirroring = true;
  /**
   * Undocumented.
   */
...
enum class ReturnCode {
  OK = 0,
  Error = 1,         // Returned on generic errors
  UserCancelled = 2, // This will be returned if a callback passed to the
                     // evaluator signalled cancelation.
  UserKilled = 3, // This will be returned if a callback passed to the evaluator
                  // signalled a killing wish.
  MissingData =
      4,         // Returned if missing data was encountered during computation
  Error_OOM = 5, // Returned if method run into an OOM situation. Will only
                 // happen if returned from RawMemoryFetcher.
  ModelFileNotPresent = 6, // Model file was not found
  Error_Inference_OOM = 7, // Returned if worker ran out of RAM / VRAM during
                           // inference.
  Error_IPC_Timeout = 8,   // Returned if IPC process timed out.
  Error_NoSupportedBackend =
      9, // Returned if model does not supported any of the requested backends
