...
public:
   ...
   * options.
   * \param device The device the memory is on. \param hash Hash of the data
   * stored in memory. Make sure that different data always has different hash
   * values, otherwise caching will return old values. If std::nullopt is passed
   * in, the hash will be computed based on the raw memory (safest, but also
   * slowest option). \param copy_memory If true, stores a copy of the passed
   * memory. Otherwise the constructed object only keeps a reference and the
   * original data_ptr memory has to be valid when accessed.
   */
  RawMemoryObject(void *data_ptr, ml_core::DataTypeEnum type,
                  ...
                  std::optional<ml_core::BlaceHash> hash = std::nullopt,
   ...
   */
  RawMemoryObject(void *data_ptr, RawMemoryMetadata meta_data,
                  bool copy_memory);
   ...
   *
   */
  RawMemoryObjectImpl *get_impl();

private:
  std::unique_ptr<blace::RawMemoryObjectImpl> impl_;
};
