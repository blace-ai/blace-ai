...
public:
   ...
   * payload_folder Payload folder. \param payload_url An optional URL to try to
   * fetch the model payload from.
   */
  InferenceOp(std::vector<char> model_bytes, std::vector<OpP> inputs,
              ...
              std::optional<std::string> payload_url = std::nullopt);
 ...
 * Operator to construct a node from custom memory.
 */
class EXPORT_OR_IMPORT FromRawMemoryFetcherOp : public BaseOp {
...
public:
   ...
   * full tracking of metadata inside the graph.
   *
   * \param mem_fetcher A shared_ptr of the memory fetcher.
   * \param device A enum indicating in which device the memory is stored.
   */
  FromRawMemoryFetcherOp(std::shared_ptr<blace::RawMemoryFetcher> mem_fetcher,
 ...
 * Operator to construct an empty node.
 */
class EXPORT_OR_IMPORT SaveImageOp : public BaseOp {
...
/**
 * Operator to save an audio file.
 */
class EXPORT_OR_IMPORT SaveAudioFileOp : public BaseOp {
...
public:
   ...
   *
   */
  SaveAudioFileOp(blace::ops::OpP input, std::string filename);
};

/**
 * Operator to slice an tensor.
 */
class EXPORT_OR_IMPORT IndexOp : public BaseOp {
public:
  /**
   * Constructs a slicing operator.
   *
   * \param input The input node.
   * \param indices A vector of indices to use for slicing.
   * \param color_format The resulting color format, optional.
   */
  IndexOp(blace::ops::OpP input, ml_core::BlaceIndexVec indices,
          std::optional<ml_core::ColorFormatEnum> color_format = std::nullopt);
};

/**
 * Constructs an operator to draw rectangles. Rectangles must come in format
 * Bx4.
 */
class EXPORT_OR_IMPORT DrawRectangles : public BaseOp {

public:
  /**
   * Constructs a rectangle-drawing operator.
   *
   * \param img The img to draw the rectangles onto.
   * \param bounding_boxes Node containing the rectangles, shape Bx4.
   ...
   */
  DrawRectangles(blace::ops::OpP img, blace::ops::OpP bounding_boxes, int r,
                 int g, int b, int thickness);
};

/**
 * Addition operation, adding either a value or another tensor.
 */
class EXPORT_OR_IMPORT AddOp : public BaseOp {
public:
  /**
   * Adds two tensors.
   *
   ...
   * \param b
   */
  AddOp(blace::ops::OpP a, blace::ops::OpP b);
  /**
   * Adds double to a tensor.
   *
   * \param a
   * \param b
   */
  AddOp(blace::ops::OpP a, double b);
  /**
   * Adds double to a tensor and sets the new value range.
   *
   * \param a
   * \param b
   * \param norm
   */
  AddOp(blace::ops::OpP a, double b, ml_core::ValueRangeEnum norm);
};

/**
 * Subtraction operation.
 */
class EXPORT_OR_IMPORT SubOp : public BaseOp {
public:
  /**
   * Subtracts one tensor from the other.
   *
   * \param a
   * \param b
   */
  SubOp(blace::ops::OpP a, blace::ops::OpP b);
};

/**
 * Sums tensors along given dimensions.
 */
class EXPORT_OR_IMPORT SumOp : public BaseOp {

public:
  /**
   * Sums tensor along dimensions.
   *
   * \param input
   * \param sizes List of dimensions to sum along.
   * \param keepdim Keep sumed dimensions?
   */
  SumOp(blace::ops::OpP input, std::vector<int64_t> sizes, bool keepdim);
};
