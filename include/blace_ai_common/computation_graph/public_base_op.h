...
public:
  BaseOp(std::shared_ptr<BaseNodeOp> impl) : _impl(impl) {}
  BaseOp() {}

  EXPORT_OR_IMPORT BaseOp &operator=(const BaseOp &other);

  EXPORT_OR_IMPORT void set_hash(ml_core::BlaceHash new_hash);

  std::shared_ptr<BaseNodeOp> getImpl() { return _impl; }
  ...
  EXPORT_OR_IMPORT ml_core::DataTypeEnum get_type();
  ...
  EXPORT_OR_IMPORT ml_core::ValueRangeEnum get_value_range();

  EXPORT_OR_IMPORT void print_types();

  /**
   * Internal storage of the constructed object.
   */
  std::shared_ptr<BaseNodeOp> _impl;
};
...
std::string serialize(std::shared_ptr<BaseOp> start_node);

std::shared_ptr<BaseOp> deserialize_to_base_op(std::string serialized_protobuf);
