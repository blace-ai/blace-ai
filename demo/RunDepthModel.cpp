...
std::shared_ptr<RawMemoryObject> raw_memory_from_file(std::string file) {
  ...
  blace::RawMemoryObject raw_mem(
      ...
      blace::ml_core::ZERO_TO_255, random_hash, true);
...
#else
  ipc::IpcEvaluator ipc_evaluator;
  ipc_evaluator.setup();
  auto eval_result = ipc_evaluator.evaluateToRawMemory(result_depth);
  ipc_evaluator.destruct();
#endif
